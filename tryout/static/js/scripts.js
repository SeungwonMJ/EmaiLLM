...
function classifyCurrentEmail() {
    ...
    setTimeout(() => {
        ...
        .then(response => {
            console.log("Classification Response Status:", response.status); // Added
            return response.json();
        })
        .then(data => {
            console.log("Classification Data:", data); // Added
        ...
        .catch(error => {
            console.error("Classification Fetch Error:", error); // Added
            resultElement.innerHTML = `
                <div class="alert alert-danger">
...
// Handles the "Process Untagged" button: confirms with user, then navigates to processing route
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (processUntaggedButton) {
        processUntaggedButton.addEventListener('click', function(event) {
            ...
            if (confirm("You are about to download and then delete all untagged emails. Are you sure you want to proceed?")) {
                window.location.href = '/process-untagged'; // Redirect to the processing route
            }
        });
    }
});
